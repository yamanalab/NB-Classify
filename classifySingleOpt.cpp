...
int main(int argc, char **argv)
{
    ...
    */
    ...
    string filename = "results/classifySingleOpt_"+dataset+to_string(class_num)+".csv";
    ofstream myfile;
    ...
    for (int i = 0; i<num; i++){
        ...
        ea.encrypt(ct_data, publicKey, data[i]);
        ...
        for (int j=1; j<class_num;j++){
            ...
            long b;
            if (diff[num_probs-1] < plain_mod/2){ b = 1; index = j;}
            else b = 0;
            vector <long> vec_b(num_slots, b);
